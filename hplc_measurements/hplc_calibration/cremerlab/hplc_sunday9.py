...
"""
        ...
        for i in  range (len(file_path)):
            ...
            raw_data.to_csv(foldername+'/converted/'+file_path[i].replace(foldername,'').replace('.txt','')+'_chromatogram.csv', index = False)
    else:
...
def process_growth_curve(file_growthdata,folder_hplcdata=None,folder_growthdata="data_growthcurves",culture_list="All",filename_settings="hplc_settings.csv",folder_peakassignment="data_peakassignment", filename_output=None,plot_time_window=None, plot_upper_limit=None,plot_lower_limit=None,calibrationfile="calibration.json",display_fullpeaklist=False):
    ...
    for il in range(0,int(grdata.shape[1]/5)):
        ...
        if culture_list=="All" or namesamplec in culture_list:
            ...
            for il3 in range(hplcname.shape[0]):
                ...
                if str(curname) != "nan":
                    with open('data_chromatogramanalysis/'+curname+'_chromatogram.json', "r") as jsonFile:
    ...
    if filename_output==None:
        filename_output='data_chromatogramanalysis/analysis_'+file_growthdata+'.csv'
