...
def convert(file_path, detector='B', delimiter=',', peak_table=False, 
        ...
        """
        Reads the ASCII output from a  HPLC and returns a DataFrame of the
        chromatogram. Converted files can also be saved to disk with human-readable 
        metadata. The standard is set to work with Shimadzu. 
        Parameters
        ----------
        file_path : str or list of str
            The file path of the ASCII file to read. Multiple files can be provided 
            as a list of file paths. 
        hplc_machine : str or None
            String to differentiate output from different HPLC machines. If None, settings for our Shimadzu HPLC (Shimadzu_ProminenceLC2030C) are used. 
        ...
        use_samplename_for_output : bool
            If True, sample name instead of filename will be used to generate output file
        """
        ...
        if hplc_machine==None or hplc_machine=="Shimadzu_ProminenceLC2030C":
        ...
        """
            ...
            for i in  range (len(file_path)):
                ...
                raw_data.to_csv(os.path.join(output_dir,file_path[i].replace('.txt','')+'.csv'), index = False)
        else:
            machine_not_defined
...
class Chromatogram(object):
    ...
    def __init__(self, file=None, time_window=None, baselinecorrection=False,
        ...
        if type(file) is str:
            ...
            dataframe = pd.read_csv(file,skiprows=15)
            display(dataframe)
        else:
            dataframe = file 
        self.df = dataframe
...
def batch_plot(file_paths, time_window=None, cols={'time':'time_min', 'intensity':'intensity_mV'},plot_upper_limit=None,plot_lower_limit=None, plot_time_window=None,detection_peakpositions=None, plot_verticallines=None,backgroundsubstraction=False, backgroundsubstraction_iterations=80,plot_characteristictimes=False, plot_output=False ):
    ...
    if plot_output != False:
        fig.savefig("plot_output/"+plot_output)
...
def read_hplcsettings(samplenames,filename_settings,calibration_settings=None):
    ...
    if calibration_settings==None:
        ...
        try: 
            conc=analysis_settings["concentration_"+calibration_settings]
        except:
...
def run_calibration(foldername,substrate,retentiontime_range,date,list_samplenames=None, name_prefix="",filename_settings="data_hplc/hplc_settings_calibration.csv",plot_time_window=None, plot_upper_limit=None,plot_lower_limit=None):
    #folder name
    #substrate: single substrate or list of substrate. e.g. glucose or ['glucose','acetate']
    #retentiontime_range (time range of substrate, list of substrates), e.g. [25,25.5] or [[25,25.5],[25,25.5]]
    #hplc_samples : provide list of hplc samples. If none, all hplc samples in file will be taken

    #get list of samples
    if list_samplenames==None:
        #get list of samples in the folder
        raw_calib_files = glob.glob('data_hplc/'+foldername+'/*.txt') #get all files
    ...
    hplc_setting=read_hplcsettings(list_samplenames,filename_settings)
    
    #make folder for converted files if folder does not exist
    pathconverted=os.path.join(os.path.join('data_hplc',foldername),"converted")
    ...
    for samplename in list_samplenames:
        ...
        if os.path.exists(samplepath_converted)==False:
            ...
            print(glob.glob(os.path.join(os.path.join('data_hplc',foldername),'*.txt')))
            convert(glob.glob(os.path.join(os.path.join('data_hplc',foldername),'*.txt')),hplc_machine=hplc_setting["hplc_machine"])
            if os.path.exists(samplepath_converted)==False:
                conversion_needed
        else:
            pass
    ...
    #plot peaks
    cal_chroms, cal_plot, cal_rettimes = batch_plot(files,time_window=hplc_setting["time_window"],plot_verticallines=hplc_setting["manual_list"],backgroundsubstraction=hplc_setting["backgroundsubstraction"],backgroundsubstraction_iterations=hplc_setting["backgroundsubstraction_iterations"],plot_time_window=plot_time_window,plot_upper_limit=plot_upper_limit,plot_lower_limit=plot_lower_limit)
                                                    
    #fit peaks
    cal_chroms, cal_peaks, cal_plot = cremerlab.hplc.batch_process(files,time_window=hplc_setting["time_window"],plot_verticallines=hplc_setting["manual_list"],backgroundsubstraction=hplc_setting["backgroundsubstraction"],backgroundsubstraction_iterations=hplc_setting["backgroundsubstraction_iterations"],plot_time_window=plot_time_window,plot_upper_limit=plot_upper_limit,plot_lower_limit=plot_lower_limit,show_visz=True)
    ...
    for iS in range(0,len(substrate)):
        ...
        name_output=name_prefix+sub+"_"+str(date) #unique name of calibration curve
        

        #select peaks according to retention time
        calibration_peak = cal_peaks[(cal_peaks['retention_time'] >= retentiontimec[0]) & (cal_peaks['retention_time'] <= retentiontimec[1])]
        print(calibration_peak['sample'].values)

        # Set up a list to store the concentrations
        concs = []
        for sample in hplc_names:
                try:
                    concs.append(hplc_setting["concentration_"+sub])
                except:
                    concs.append(hplc_setting["concentration"])
        
        # Loop through each sample name tand get concentration from file name. 
        # In this example, scfa_2mM001_chromatogram.csv
        for s in calibration_peak['sample'].values:

            split = s.split('_')  # Split the file name by underscores. Output: ['scfa','2mM001','chromatogram.csv']
        ...
        with open("data_hplc/calibration.json", "r") as jsonFile:
            data = json.load(jsonFile)
        data[name+'_'+date+'_slope'] = slope
        data[name+'_'+date+'_intercept']=intercept
        outjson='data_hplc/calibration.json'
...
#    convert(raw_hplc_files)
#    
#    raw_calib_files = glob.glob('data_hplc/'+foldername+'/*.txt') #this gives list of the files
#cremerlab.hplc.convert(raw_calib_files) #convert into simple txt iles with chromatogram info (and some metadata as comments)
    ...
    for samplename in list_samplenames:
        samplepath_converted=os.path.join(pathconverted,samplename+"_chromatogram.csv")
        if os.path.exists(samplepath_converted)==False:
            convert(glob.glob(os.path.join(foldername,'*.txt')),hplc_machine=hplc_settings["hplc_machine"])
            if os.path.exists(samplepath_converted)==False:
                conversion_needed
        else:
            pass
