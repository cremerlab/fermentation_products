...
fps_enthalpies = { 'acetate': 0.21, 'butyrate': 0.52, 'formate': 0.0, 'lactate': 0.33, 'propionate': 0.37, 'succinate': 0.36}
...
_data = data[(data['age_category']=='Adult') & (data['disease_state']=='Healthy')]
biomass_hist, biomass_bins = np.histogram(_data['drymass'], bins=INIT_BINS)
total_fp_hist, total_fp_bins = np.histogram(_data['total_excretion_mass'], bins=INIT_BINS)
composition_hist, composition_bins = np.histogram(_data['bm_fraction'], bins=INIT_BINS)
...
data_table = bokeh.models.DataTable(source=table_cds, columns=columns, width=850, height=300)
...
totals_grid = bokeh.layouts.gridplot([[biomass_ax], [total_fp_ax], [total_energy_ax]])
